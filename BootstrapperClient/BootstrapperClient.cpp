...
static const TCHAR* BootstrapperFileName    = _T("AisakaLauncher.exe");
...
LPCWSTR robloxUpdaterTaskName = L"CaelusGameUpdater";
...
bool BootstrapperClient::NeedPreDeployRun()
{
	...
	CRegKey key;
	if (SUCCEEDED(key.Open(HKEY_CURRENT_USER, _T("Software\\Aisaka\\Caelus"), KEY_READ)))
	{
		TCHAR buf[MAX_PATH];
...
void BootstrapperClient::RunPreDeploy()
{
	...
	try
	{
		...
		CRegKey key;
		if (SUCCEEDED(key.Create(HKEY_CURRENT_USER, _T("Software\\Aisaka\\Caelus"))))
		{
...
void BootstrapperClient::StartRobloxApp(bool fromInstall)
{
	...
	if (!playArgs)
	{
		...
		if (!isIDE)
		{
			...
			{
				if (url.find(_T("www.")) != 0 && url.find(_T("https:")) != 0) 
				{
...
void BootstrapperClient::DeployComponents(bool isUpdating, bool commitData)
{
	...
	files.push_back(std::pair<std::wstring, std::wstring>(_T("AisakaPlayer.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("ExtraContent.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("PlatformContent.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("PlayerContent.zip"), _T("")));
	files.push_back(std::pair<std::wstring, std::wstring>(_T("Shaders.zip"), _T("")));
