...
void Bootstrapper::RegisterUninstall(const TCHAR *productName)
{
	...
	throwHRESULT (keyProductCode.SetStringValue(_T("URLInfoAbout"), _T("https://www.caelus.lol")), "Failed to set URLInfoAbout key");
...
void Bootstrapper::RegisterProtocolHandler(const std::wstring& protocolScheme, const std::wstring& exePath, const std::wstring& installKeyRegPath)
{
	...
	throwHRESULT(key.SetStringValue(_T(""), _T("URL: I hope this dies")), format_string("failed to set value for protocol"));
...
void Bootstrapper::validateAndFixChromeState()
{
	...
	for (rapidjson::Value::MemberIterator  iter = es.MemberBegin(); iter != es.MemberEnd(); ++iter)
	{
		if (iter->name.IsString())
		{
			...
			if (name.find("caelus") != std::string::npos)
			{
...
{
    std::string text = convert_w2s(GetFriendlyName()) + std::string(isUpdating ? " をアップグレード中…" : " をインストール中…") + " ...";
	message(text);
	progressBar(0, 100);
		...
		if(moduleVersionNumber != bootstrapperVersion){
			...
			catch (...)
			{
				newBootstrapper = simple_logger<wchar_t>::get_temp_filename(_T("tmp"));		// We could use an "exe" extension, but hiding the type isn't a bad idea?
				{
					...
					HttpTools::httpGetCdn(this, installHost, format_string("/%s-Aisaka.exe", installVersion.c_str()), std::string(), bootstrapperFile, false, boost::bind(&Bootstrapper::dummyProgress, _1, _2));
				}
			}
...
bool Bootstrapper::shutdownProgress(int& time, int pos, int max)
{
	...
	if (time==1000)
	{
        std::string text = convert_w2s(GetFriendlyName()) + "をシャットダウン中…";
		message(text);
		dialog->SetMarquee(false);
	}
	progressBar(pos, max);
...
void Bootstrapper::run()
{
	...
	try
	{
		...
		bool isNewInstall = false;
		{
			...
			if (isUninstall)
			{
                std::string text = convert_w2s(GetFriendlyName()) + "をアンインストール中…";
				message(text);
				uninstall();
				if (windowed)
				{
        ...
        std::string text = convert_w2s(GetFriendlyName()) + " は最新の状態です。";
		message(text);
		if (debug)
		{
			dialog->SetMarquee(false);
...
void Bootstrapper::install()
{
	...
	try
	{
        ...
        std::string text = convert_w2s(GetFriendlyName()) + "を設定中…";
		message(text);
...
void Bootstrapper::setLatestProcess()
{
	...
	const std::string latestProcessName = format_string("www.caelus.lol/%s/%s/latestProcess", installHost.c_str(), Type().c_str());
